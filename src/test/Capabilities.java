...
//		For iOS v > 10.2
    ...
    public static AndroidDriver<AndroidElement> androidEmulatorCapabilities(String device, String appName) throws IOException {
        Properties properties = new Properties();
        InputStream input = new FileInputStream(System.getProperty("user.dir") + "/src/test/global.properties");
        properties.load(input);

        String APP_NAME = properties.getProperty(appName);

        File file = new File("src");
//		File fapk = new File(file,"ApiDemos-debug.apk");
        File fapk = new File(file, properties.getProperty(appName));
        DesiredCapabilities cap = new DesiredCapabilities();
        cap.setCapability(MobileCapabilityType.DEVICE_NAME, device);
